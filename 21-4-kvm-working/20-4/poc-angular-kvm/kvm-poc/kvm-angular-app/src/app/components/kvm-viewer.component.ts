...
@Component({
  ...
  template: `
    <div class="kvm-viewer" (click)="focusCanvas()">
      ...
      <!-- Overlay: shown while waiting for the first screen frame.
           BIOS startup can take up to 8 minutes before AMT sends any pixels. -->
      <div class="no-data-overlay" *ngIf="showNoDataOverlay">
        <div class="no-data-box">
          <div class="no-data-spinner"></div>
          <div class="no-data-title">Waiting for device screen…</div>
          <div class="no-data-msg">
            BIOS / firmware startup may take <strong>up to 8 minutes</strong>
            before the screen appears. The session is active — please wait.
          </div>
          <div class="no-data-state">RFB: {{ rfbState }}</div>
        </div>
      </div>

      <div class="kvm-toolbar" *ngIf="rfbState === 'Normal' && !showNoDataOverlay">
...
export class KvmViewerComponent implements OnInit, OnDestroy, OnChanges {
  ...
  // Overlay shown while no RFB frame has arrived yet (BIOS boot can take up to 8 min).
  // Set to false by renderRaw() on the very first pixel frame received.
  ...
  resetRfbState(): void {
    ...
    // Show the overlay again for the new session — BIOS may need up to 8 min.
    this.showNoDataOverlay = true;
  ...
  private renderRaw(x: number, y: number, w: number, h: number, data: Uint8Array): void {
    ...
    // First real pixel received — dismiss the "BIOS starting up" overlay.
    if (this.showNoDataOverlay) {
      this.showNoDataOverlay = false;
      this.cdr.detectChanges();
    }
  }
