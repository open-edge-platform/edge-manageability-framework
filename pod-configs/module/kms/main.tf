...
# Reference existing statically created IAM user
data "aws_iam_user" "vault" {
  user_name = "vault-user"
}

# Create new access key for the existing user
resource "aws_iam_access_key" "vault" {
  user = data.aws_iam_user.vault.user_name
}
