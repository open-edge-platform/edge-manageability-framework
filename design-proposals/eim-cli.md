...
> NOTE: if autoprovisioning is set up in provider the onboarded host will provision automatically this should be communicated to user
...
> TODO not all fields in OS profile are updatable (really only ubuntu profile are updateable)
