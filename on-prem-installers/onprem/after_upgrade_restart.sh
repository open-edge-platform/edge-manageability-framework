...
check_and_download_dkam_certs() {
    ...
    while (( attempt <= max_attempts )); do
        ...
        if wget https://pa-server-nginx."$CLUSTER_DOMAIN"/tink-stack/keys/Full_server.crt --no-check-certificate --no-proxy -q -O Full_server.crt 2>/dev/null; then
            echo "$(green)[OK] Full_server.crt downloaded successfully$(reset)"
            
            # Try to download signed_ipxe.efi using the certificate
            if wget --ca-certificate=Full_server.crt https://pa-server-nginx."$CLUSTER_DOMAIN"/tink-stack/signed_ipxe.efi -q -O signed_ipxe.efi 2>/dev/null; then
