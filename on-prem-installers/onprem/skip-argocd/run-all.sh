...
COMPONENTS=(
  ...
  "istio-base:istio-base istio-base.sh"
#  "k8s-metrics-server:k8s-metrics-server k8s-metrics-server.sh"
  "prometheus-crd:prometheus-crd prometheus-crd.sh"
  "istiod:istiod istiod.sh"
  "reloader:reloader deploy-reloader.sh"
  "wait-istio-job:wait-istio-job wait-istio-job.sh"
  "postgresql-secrets:postgresql-secrets postgresql-secrets.sh"
  "postgresql-cluster:postgresql-cluster postgresql-cluster.sh"
  "istio-policy:istio-policy istio-policy.sh"
  "kiali:kiali kiali.sh"
  "platform-autocert:platform-autocert platform-autocert.sh"
  "platform-keycloak:platform-keycloak platform-keycloak.sh"
  "cert-synchronizer:cert-synchronizer cert-synchronizer.sh"
