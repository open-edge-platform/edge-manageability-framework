...
#DebugError platform-keycloak dependency  "copy-keycloak-admin-to-infra:copy-keycloak-admin-to-infra copy-keycloak-admin-to-infra.sh"
...
run_component() {
  ...
  if [[ -d "$dir" && -f "$dir/$script" ]]; then
    ...
    echo "⏱️ [$name] Start: $START_HUMAN"
    echo "⏱️ [$name] End  : $END_HUMAN"
    echo "⏱️ [$name] Took : ${DURATION}s"
    echo "----------------------------------------"
