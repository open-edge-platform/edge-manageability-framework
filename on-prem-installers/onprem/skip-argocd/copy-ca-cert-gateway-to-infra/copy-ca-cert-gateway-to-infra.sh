...
RELEASE_NAME="copy-ca-cert-gateway-to-infra"
...
verify_deployment() {
    ...
    kubectl get externalsecrets,secretstores -n $NAMESPACE -o wide
...
create_source_secret() {
    ...
    if ! kubectl get secret source-name -n $NAMESPACE >/dev/null 2>&1; then
