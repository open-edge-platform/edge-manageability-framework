...
write_shared_variables() {
  ...
  # Update SRE TLS configuration in place
  if [[ -n "${SRE_TLS_ENABLED:-}" && "${SRE_TLS_ENABLED}" != "false" ]]; then
    if grep -q "^export SRE_TLS_ENABLED=" "$config_file"; then
      sed -i "s|^export SRE_TLS_ENABLED=.*|export SRE_TLS_ENABLED='${SRE_TLS_ENABLED}'|" "$config_file"
    else
      echo "export SRE_TLS_ENABLED='${SRE_TLS_ENABLED}'" >> "$config_file"
    fi
  fi
  
  if [[ -n "${SRE_DEST_CA_CERT:-}" ]]; then
    if grep -q "^export SRE_DEST_CA_CERT=" "$config_file"; then
      sed -i "s|^export SRE_DEST_CA_CERT=.*|export SRE_DEST_CA_CERT='${SRE_DEST_CA_CERT}'|" "$config_file"
    else
      echo "export SRE_DEST_CA_CERT='${SRE_DEST_CA_CERT}'" >> "$config_file"
    fi
  fi
  
  # Update SMTP configuration in place
  if [[ -n "${SMTP_SKIP_VERIFY:-}" && "${SMTP_SKIP_VERIFY}" == "true" ]]; then
    if grep -q "^export SMTP_SKIP_VERIFY=" "$config_file"; then
      sed -i "s|^export SMTP_SKIP_VERIFY=.*|export SMTP_SKIP_VERIFY='${SMTP_SKIP_VERIFY}'|" "$config_file"
    else
      echo "export SMTP_SKIP_VERIFY='${SMTP_SKIP_VERIFY}'" >> "$config_file"
    fi
  fi
  
  # Update profile disable flags in place
  if [[ -n "${DISABLE_CO_PROFILE:-}" && "${DISABLE_CO_PROFILE}" == "true" ]]; then
    if grep -q "^export DISABLE_CO_PROFILE=" "$config_file"; then
      sed -i "s|^export DISABLE_CO_PROFILE=.*|export DISABLE_CO_PROFILE='${DISABLE_CO_PROFILE}'|" "$config_file"
    else
      echo "export DISABLE_CO_PROFILE='${DISABLE_CO_PROFILE}'" >> "$config_file"
    fi
  fi
  
  if [[ -n "${DISABLE_AO_PROFILE:-}" && "${DISABLE_AO_PROFILE}" == "true" ]]; then
    if grep -q "^export DISABLE_AO_PROFILE=" "$config_file"; then
      sed -i "s|^export DISABLE_AO_PROFILE=.*|export DISABLE_AO_PROFILE='${DISABLE_AO_PROFILE}'|" "$config_file"
    else
      echo "export DISABLE_AO_PROFILE='${DISABLE_AO_PROFILE}'" >> "$config_file"
    fi
  fi
  
  if [[ -n "${DISABLE_O11Y_PROFILE:-}" && "${DISABLE_O11Y_PROFILE}" == "true" ]]; then
    if grep -q "^export DISABLE_O11Y_PROFILE=" "$config_file"; then
      sed -i "s|^export DISABLE_O11Y_PROFILE=.*|export DISABLE_O11Y_PROFILE='${DISABLE_O11Y_PROFILE}'|" "$config_file"
    else
      echo "export DISABLE_O11Y_PROFILE='${DISABLE_O11Y_PROFILE}'" >> "$config_file"
    fi
  fi
