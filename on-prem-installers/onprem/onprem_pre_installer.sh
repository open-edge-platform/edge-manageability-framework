...
write_shared_variables() {
  ...
  echo "# Shared configuration variables for onprem installer scripts" > "$config_file"
  echo "# Generated by onprem_pre_installer.sh on $(date)" >> "$config_file"
  echo "" >> "$config_file"
  
  # Write environment variables that are used by onprem_installer.sh
  echo "export RELEASE_SERVICE_URL='${RELEASE_SERVICE_URL}'" >> "$config_file"
  echo "export ORCH_INSTALLER_PROFILE='${ORCH_INSTALLER_PROFILE}'" >> "$config_file"
  echo "export DEPLOY_VERSION='${DEPLOY_VERSION}'" >> "$config_file"
  
  # Write command line argument variables
  echo "export SRE_TLS_ENABLED='${SRE_TLS_ENABLED:-false}'" >> "$config_file"
  if [[ -n "${SRE_DEST_CA_CERT:-}" ]]; then
    # Properly escape the certificate content for shell
    echo "export SRE_DEST_CA_CERT='${SRE_DEST_CA_CERT}'" >> "$config_file"
  fi
  echo "export ENABLE_TRACE='${ENABLE_TRACE:-false}'" >> "$config_file"
  
  echo "" >> "$config_file"
  echo "# Working directory where script was executed" >> "$config_file"
  echo "export ONPREM_WORKING_DIR='${cwd}'" >> "$config_file"
