...
install_gitea_from_repo() {
  ...
  randomPassword() { tr -dc A-Za-z0-9 </dev/urandom | head -c 16; }
  ...
  createGiteaAccount() {
    ...
    local giteaPod
    giteaPod="$(kubectl get pods -n gitea -l app=gitea -o jsonpath='{.items[0].metadata.name}')"
    if [[ -z "$giteaPod" ]]; then
...
# Decide whether to install Gitea based on generated config flags
