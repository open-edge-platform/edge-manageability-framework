...
## Layer 2 — Angular Dev Proxy

The Angular dev server (`npm start`) uses `proxy.conf.json` to forward requests
...
This endpoint is a read-only health probe. It is safe to call at any time —
it never modifies state and always returns `200 OK`.

**What it returns:**

```go
type StatusResponse struct {
    State   string   `json:"state"`   // current connection state (see below)
    Logs    []string `json:"logs"`    // recent protocol log messages (last ~500 lines)
    Device  string   `json:"device"`  // GUID of the currently connected AMT device
    MPSHost string   `json:"mpsHost"` // hostname of the MPS server being used
}
```
...
| `state` value | Meaning |
|---|---|
| `"disconnected"` | No active session. Either `POST /api/connect` was never called, or the session was closed/errored and `POST /api/disconnect` cleaned it up. |
| `"connecting"` | `POST /api/connect` has been called and the Go server is establishing the MPS WebSocket. The redirect token exchange and TCP handshake are in progress. |
| `"authenticating"` | The MPS WebSocket is open. The AMT Redirect handshake is running (RedirectStart → DigestAuth sequence). |
| `"active"` | The AMT ChannelOpen message was sent successfully. Raw RFB bytes are now flowing. The browser `/ws` WebSocket may or may not be attached yet. |
| `"error"` | A fatal error occurred (e.g., bad credentials, MPS unreachable, AMT rejected auth). The session object is kept temporarily so the error can be read from `logs`. |

**How the handler works — step by step:**

**Step 1 — Read session under a read lock**
```go
srv.mu.RLock()
session := srv.session
srv.mu.RUnlock()
```
The server's single `*KVMSession` pointer is read atomically under a read-lock
so reads never block `POST /api/connect` or `POST /api/disconnect`, which hold
the write-lock only briefly.

**Step 2 — Build response with a safe default**
```go
resp := StatusResponse{
    State: "disconnected",
    Logs:  srv.getLogs(),
...
```
If `session` is `nil` (no connection has ever been attempted), the response is
immediately ready with `state: "disconnected"` and the current log buffer.
`getLogs()` copies the ring buffer under its own mutex so the response is a
snapshot — log entries arriving concurrently are not included.

**Step 3 — Override defaults if a session exists**
```go
if session != nil {
    resp.State   = session.getState()   // read state under session's own stateMu
    resp.Device  = session.deviceGUID
    resp.MPSHost = srv.config.MPSHost
}
```
`getState()` acquires `session.stateMu` with a read-lock — it is safe to call
while relay goroutines may be updating the state concurrently.
`Device` and `MPSHost` are set once during `handleConnect()` and never mutated
afterwards, so they can be read without a lock.
...
```
Always `200 OK`. The caller determines health from the `state` field, not the
HTTP status code.
...
No session yet:
```json
{ "state": "disconnected", "logs": [], "device": "", "mpsHost": "" }
```

AMT Redirect handshake in progress:
```json
{ "state": "authenticating", "logs": ["[AMT] Sending RedirectStart", "[AMT] RedirectStart reply OK"], "device": "a1b2-...", "mpsHost": "mps.example.com" }
```

Session live and streaming video:
```json
{ "state": "active", "logs": ["[RFB] ServerInit 1280x1024", "[RFB] Received FramebufferUpdate"], "device": "a1b2-...", "mpsHost": "mps.example.com" }
```

**How the Angular UI uses it:**

`kvm.service.ts` polls `GET /api/status` every few seconds during connection
setup (before the WebSocket is confirmed open) so `app.ts` can show a
human-readable status string in the UI. Once the WebSocket is open and the
`state` field reads `"active"`, polling stops and the real-time canvas takes over.

**Locking summary:**

```
handleStatus()
    srv.mu.RLock()          ← prevents a concurrent handleConnect() / handleDisconnect()
    session ptr copy
    srv.mu.RUnlock()

    srv.getLogs()
        srv.logsMu.Lock()   ← copy log ring buffer
        srv.logsMu.Unlock()

    session.getState()
        stateMu.RLock()     ← read state safely while relay goroutines run
        stateMu.RUnlock()
```

All three locks are held only for microseconds.
`handleStatus()` will never deadlock and will never block the data-plane goroutines.
...
**Timeline showing when queuing matters:**

```
t=0ms   POST /api/connect → Go server starts AMT handshake
t=50ms  AMT Redirect handshake complete, state = "active"
t=55ms  AMT sends "RFB 003.008\n"  → sendToBrowser() → browserConn==nil → QUEUED
t=60ms  200 OK returned to browser
t=110ms Browser opens WebSocket /ws
t=111ms handleKVMWebSocket() sets browserConn = conn
t=111ms flushPendingBrowserFrames() sends "RFB 003.008\n" → Browser
t=111ms Browser receives "RFB 003.008\n", RFB handshake begins normally
```

Without the queue, the 50 ms window at t=55ms would silently drop the RFB version
string and the browser's state machine would wait forever for data that already passed.
