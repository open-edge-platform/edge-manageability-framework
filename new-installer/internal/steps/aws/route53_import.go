...
const (
    RT53ModulePath       = "new-installer/targets/aws/iac/route53"
    RT53BackendBucketKey = "rt53.tfstate"
)
var rt53StepLabels = []string{
    "aws",
    "rt53",
}
type RT53Variables struct {
    CustomerTag      string   `json:"customer_tag,omitempty"`
    ParentZone       string   `json:"parent_zone,omitempty"`
    OrchName         string   `json:"orch_name,omitempty"`
    VPCID            string   `json:"vpc_id,omitempty"`
    VPCRegion        string   `json:"vpc_region,omitempty"`
    Hostname         []string `json:"hostname,omitempty"`
    Traefik2Hostname []string `json:"traefik2_hostname,omitempty"`
}
// NewDefaultRT53Variables creates a new RT53Variables with default values
// based on variable.tf default definitions.
func NewDefaultRT53Variables() RT53Variables {
    return RT53Variables{
        CustomerTag:      "",
        ParentZone:       "",
        OrchName:         "",
        VPCID:            "",
        VPCRegion:        "",
        Hostname:         []string{},
        Traefik2Hostname: []string{},
    }
}
type RT53Step struct {
    variables          RT53Variables
...
func CreateRT53Step(rootPath string, keepGeneratedFiles bool, terraformUtility steps.TerraformUtility, awsUtility AWSUtility) *RT53Step {
    return &RT53Step{
        RootPath:           rootPath,
        KeepGeneratedFiles: keepGeneratedFiles,
        TerraformUtility:   terraformUtility,
        AWSUtility:         awsUtility,
        StepLabels:         rt53StepLabels,
    }
}
func (s *RT53Step) Name() string {
    return "RT53Step"
}
func (s *RT53Step) Labels() []string {
    return s.StepLabels
}
func (s *RT53Step) ConfigStep(ctx context.Context, config config.OrchInstallerConfig, runtimeState config.OrchInstallerRuntimeState) (config.OrchInstallerRuntimeState, *internal.OrchInstallerError) {
    s.variables = NewDefaultRT53Variables()
    s.variables.VPCRegion = config.AWS.Region
    s.variables.ParentZone = config.AWS.ParentZone
    s.variables.OrchName = config.Global.OrchName
    s.variables.Hostname = config.AWS.Hostname
    s.variables.Traefik2Hostname = config.AWS.Traefik2Hostname
...
func (s *RT53Step) PreStep(ctx context.Context, config config.OrchInstallerConfig, runtimeState config.OrchInstallerRuntimeState) (config.OrchInstallerRuntimeState, *internal.OrchInstallerError) {
    if s.skipRT53Step(config) {
        return runtimeState, nil
    }
...
func (s *RT53Step) RunStep(ctx context.Context, config config.OrchInstallerConfig, runtimeState config.OrchInstallerRuntimeState) (config.OrchInstallerRuntimeState, *internal.OrchInstallerError) {
    if s.skipRT53Step(config) {
        return runtimeState, nil
    }
...
func (s *RT53Step) PostStep(ctx context.Context, config config.OrchInstallerConfig, runtimeState config.OrchInstallerRuntimeState, prevStepError *internal.OrchInstallerError) (config.OrchInstallerRuntimeState, *internal.OrchInstallerError) {
    if s.skipRT53Step(config) {
        return runtimeState, nil
    }
...
func (s *RT53Step) skipRT53Step(config config.OrchInstallerConfig) bool {
